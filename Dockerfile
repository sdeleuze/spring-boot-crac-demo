...
ADD "https://cdn.azul.com/zulu/bin/zulu17.42.19-ca-crac-jdk17.0.7-linux_x64.tar.gz" $JAVA_HOME/openjdk.tar.gz
RUN tar --extract --file $JAVA_HOME/openjdk.tar.gz --directory "$JAVA_HOME" --strip-components 1; rm $JAVA_HOME/openjdk.tar.gz;
COPY --from=antonkozlov/sb-crac /opt/jdk/lib/criu /opt/jdk/lib/criu
RUN mkdir -p /opt/app
